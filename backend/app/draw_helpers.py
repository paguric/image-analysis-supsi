...
def draw_contours_on_image(img, contour_map):
    ...
    for i, (center, cnt) in enumerate(contour_map.items(), start=1):
        ...
        draw_label(black_img, i, position=(center[0], center[1] - 10), color=parcon.YELLOW)
...
def draw_matched_contours(
    ...
    for idx in matched_left:
        ...
        draw_circle(output, center=center_l, radius=w//2, color=parcon.BLUE, filled=False)
        draw_label(output, idx, (center_l[0] - h//2, center_l[1] - h//2), parcon.BLUE, 0.6, 2)

        x, y, w, h = cv2.boundingRect(contour_r)
        draw_circle(output, center=center_l, radius=w//2, color=parcon.RED, filled=False)
        draw_label(output, idx, (center_l[0] + h//2, center_l[1] + h//2), parcon.RED, 0.6, 2)
