...
def threshold_test(input_video_path):
    ...
    cv2.imwrite(f'out/threshold_test/initial_image.jpg', frame)

    # convert the image to grayscale format
    img_gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    cv2.imwrite(f'out/threshold_test/initial_image_grayscale.jpg', img_gray)

    for i in range(255):
        # apply binary thresholding
        ret, thresh = cv2.threshold(img_gray, i, 255, cv2.THRESH_BINARY)

        cv2.imwrite(f'out/threshold_test/image_thres_{i}.jpg', thresh)
...
def brightness_test(input_video_path):
    ...
    """
    os.makedirs('out/brightness_test', exist_ok=True)
    ...
    frame = cv2_utils.extract_frame(input_video_path, middle_frame_idx)
    cv2.imwrite('out/brightness_test/initial_image.jpg', frame)

    for i in range(-127, 128):  # da -127 a +127 inclusi
        bright = cv2.convertScaleAbs(frame, alpha=1, beta=i)
        cv2.imwrite(f'out/brightness_test/image_bright_{i}.jpg', bright)
...
def contrast_test(input_video_path):
    ...
    """
    os.makedirs('out/contrast_test', exist_ok=True)
    ...
    frame = cv2_utils.extract_frame(input_video_path, middle_frame_idx)
    cv2.imwrite('out/contrast_test/initial_image.jpg', frame)
    ...
    for alpha in np.arange(0.0, 3.01, 0.01):
        ...
        cv2.imwrite(f'out/contrast_test/image_contrast_{alpha_str}.jpg', contrasted)
