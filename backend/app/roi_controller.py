...
def compute_aligned_roi_diff(
    ...
    output = np.zeros_like(cv2_utils.extract_frame(roi_prima[0].video_path, frame), dtype=np.float32)

    for roi_l, roi_r in zip(roi_prima, roi_dopo):

        patch_prima = roi_l.get_pixels()
        patch_dopo = roi_r.get_pixels()
        ...
        p1_centered = center_patch_on_canvas(patch_prima, common_size).astype(np.float32)
        p2_centered = center_patch_on_canvas(patch_dopo, common_size).astype(np.float32)

        # cv2.subtract è come scrivere p2_centered - p1_centered
        diff = cv2.subtract(p2_centered - p1_centered)

        # Maschera circolare per escludere gli angoli
        radius = common_size // 2
        maschera = np.zeros((common_size, common_size), dtype=np.uint8)
        cv2.circle(maschera, (radius, radius), radius, 255, -1) # il raggio è anche il centro
