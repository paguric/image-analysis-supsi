...
from backend.app.helpers import cv2_utils


def test_time_convert():
    assert cv2_utils.time_convert(0) == "0:00"
    assert cv2_utils.time_convert(65) == "1:05"
    assert cv2_utils.time_convert(3600) == "60:00"
...
def test_plot_histogram(mocker):
    ...
    cv2_utils.plot_histogram(frame_test)
...
def test_brightest_frame(video_test):
    indice, luminosita = cv2_utils.brightest_frame(video_test)
...
def test_extract_frame_valido(video_test):
    frame = cv2_utils.extract_frame(video_test, 1)
...
def test_extract_frame_fuori_limite(video_test):
    frame = cv2_utils.extract_frame(video_test, 999)
